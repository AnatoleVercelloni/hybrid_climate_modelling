...
#557 input scalars
vars_mli      = ['state_t','state_q0001',
                     'state_ps', 'pbuf_SOLIN','pbuf_LHFLX', 'pbuf_SHFLX']

vertcial_vars = ['state_t','state_q0001', 'ptend_t','ptend_q0001']                  


#368 output scalars
vars_mlo      = ['ptend_t','ptend_q0001',
...
def task(i):
    ...
    for var in ds.variables:
        if str(var) in vertcial_vars:
            L_i.append(np.array(ds[str(var)]))
        else:
            L_i.append(np.reapeat(ds[str(var)]), 60)

    L_o = []

    for var in dso.variables:
        if str(var) in vertcial_vars:
            L_o.append(np.array(dso[str(var)]))
        else:
            L_o.append(np.reapeat(dso[str(var)]), 60)



    ds  = np.concatenate(L_i)
    dso = np.concatenate(L_o)
    ...
    np.save('/gpfsscratch/rech/psl/upu87pm/my_preprocessed_data/v1/'+set_+'/'+str(n_npy)+'/shuffle_reduced_input_'+str(i).rjust(2,'0')+'.npy', ds)
    np.save('/gpfsscratch/rech/psl/upu87pm/my_preprocessed_data/v1/'+set_+'/'+str(n_npy)+'/shuffle_reduced_target_'+str(i).rjust(2,'0')+'.npy', dso)
